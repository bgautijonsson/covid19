Make_Stan_Data <- function(min_cases = 50, start_date = NULL, upper_mult = 2) {
    library(tidyverse)
    library(googlesheets4)
    library(readxl)
    library(lubridate)
    
    if (is_null(start_date)) start_date <- ymd("2020-03-10")
    ...
    stan_data %>% 
        ...
        mutate(days = as.numeric(date - min(date)),
               n_days = n()) %>% 
        filter(min(total_cases) > min_cases,
               min(date) == start_date,
               max(date) == end_date) %>% 
        ungroup %>% 
