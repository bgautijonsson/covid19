...
d_age <- read_csv(data_path_age) %>% mutate(age_type = "Hermun")
...
* Mjög lítil hliðrun á aldursdreifingu í átt að eldri smituðum leiðir til meiri álags á heilbrigðiskerfið.
...
Í töflu 1 að neðan má sjá muninn á aldursdreifingu smitaðra og íbúa á Íslandi yfirhöfuð. Niðurstöður hingað til sýna að börn smitast ekki jafn gjarnan og fullorðin. Búum því til hermunardreifinguna á eftirfarandi máta:

* Höldum smitadreifingunni í aldurshópum `[30 - 39]` og yngri.
* Fyrir ofan þennan aldur notum við aldursdreifingu íbúa íslands.
* Þessi dreifing summar ekki endilega upp í 100%, svo við deilum í hana með summunni svo að hún summist í 100%.

```{r}
aldur %>% 
  select(aldur, flat_dreifing_aldur_iceland, dreifing_aldur_data, p_tilfelli) %>% 
  set_names(c("Aldur", "Íbúa", "Greindra smita", "Hermun")) %>% 
  ...
  add_header_above(c("", "Dreifing" = 3))
```

```{r}
aldur %>% 
  select(aldur, dreifing_aldur_data, p_tilfelli) %>% 
