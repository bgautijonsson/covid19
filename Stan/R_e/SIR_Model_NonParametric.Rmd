...
m <- sampling(stan_model("SIR_Model_Discrete_NonParametric.stan"), 
              data  = stan_data, chains = 4, iter = 10000, warmup = 5000)
```
...
```{r}
tidyMCMC(m, pars = c("rho", "alpha", "phi_inv_sqrt", "sigma_beta", "r0", "r[69]"), 
...
```{r}
curve(dgamma(x, 4.95, 0.66), from = 0, to = 30)
```
...
tidyMCMC(m, pars = "r", conf.int = T, rhat = T, ess = T) %>% 
    ...
    background_grid(major = "none", minor = "none") +
    ggsave("r0_iceland.png", width = 6, height = 0.5 * 6, scale = 2.5)
```
