...
model {
  ...
  new_cases ~ poisson(growth[country] .* old_cases_vec + log(pop[country] - old_cases_vec));
...
generated quantities {
  ...
  int<lower = 0, upper = 300000> pred_cases[simulation_days];
  ...
  pred_cases[1] = start_value + poisson_rng(growth[simulation_country] * start_value + log(pop[simulation_country] - start_value)) - pred_recovered[1] - pred_deaths[1];
  ...
  for (i in 2:simulation_days) {
    ...
    pred_cases[i] = pred_cases[i - 1] + poisson_rng(growth[simulation_country] * pred_cases[i - 1] +
                    log(pop[simulation_country] - pred_cases[i - 1])) - 
                    pred_recovered[i] - pred_deaths[i];
  }
}
