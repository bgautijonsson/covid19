...
saved_dates <- list.files("Past_Models_GenLogistic") %>% 
...
make_gif <- function(chosen_country) {
  for (i in seq_along(saved_dates)) {
    d <- str_c("Past_Models_GenLogistic/Stan_Data_", saved_dates[i], ".csv") %>% 
      read_csv
  ...
  for (i in seq_along(saved_dates)[-dates_drop]) {
    m <- str_c("Past_Models_GenLogistic/Model_", saved_dates[i], ".rds") %>% 
      read_rds
    d <- str_c("Past_Models_GenLogistic/Stan_Data_", saved_dates[i], ".csv") %>% 
      read_csv
  ...
  anim_save(str_c("GIF/", chosen_country, "_past_fits_GenLogistic.gif"), 
            animate(p, width = 1000, height = 0.5 * 1000))
}
```
