...
p <- d %>% 
  ...
  labs(x = "Days since rate reached 0.02",
       y = "Cases per 1000",
...
results %>% 
  ungroup %>% 
  filter(par %in% c("beta", "alpha", "maximum", "beta2")) %>% 
...
results %>% 
  ungroup %>% 
  filter(par %in% c("mu_beta", "sigma_sq_beta", "sigma_sq_beta2", "mu_alpha", "sigma_sq_alpha", "beta_a", "beta_b", "mu_phi_inv")) %>% 
  mutate(par = str_to_title(par)) %>% 
