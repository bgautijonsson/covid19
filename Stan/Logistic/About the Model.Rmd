...
results <- tidyMCMC(m, conf.int = T, rhat = T, ess = T, estimate.method = "median", conf.method = "quantile") %>% 
    mutate(par = str_match(term, "[a-z_]+")) %>% 
    group_by(par) %>% 
...
However, there is a lot of implicit correlation in the values of $I_{i, t}$ for differet values of $t$. Thus, a better parametrization would be to model the daily number of cases. 
...
so that the change in rates between days is
...
Then, conditional on a Poisson likelihood and population $E_i$, the daily number of observed cases, $D_i$, can be written as
...
* Only keep data for which cases per 1000 inhabitants $\geq$ 0.04.
* Only keep data for countries for which the first observation after this filtering is $leq$ 0.08
...
* Only keep data for countries with at least 7 days of follow-up after the above steps.
...
p <- d %>% 
    ...
    labs(x = "Days since rate reached 0.04",
         y = "Cases per 1000",
...
```{r}
 results %>% 
    ungroup %>% 
    filter(par %in% c("beta", "alpha", "maximum", "phi_inv")) %>% 
...
results %>% 
    ungroup %>% 
    filter(par %in% c("mu_beta", "sigma_sq_beta", "mu_alpha", "sigma_sq_alpha", "beta_a", "beta_b", "mu_phi_inv")) %>% 
    mutate(par = str_to_title(par)) %>% 
