...
stan_data <- list(N_obs = N_obs, N_countries = N_countries, max_case_rate = max_case_rate,
                  ...
                  country_max = country_max, country_min = country_min)
str(stan_data)
```
...
```{r}
tidyMCMC(m, pars = c("mu_alpha", "sigma_sq_alpha", "mu_beta", "sigma_sq_beta", "a_sigma_obs", "b_sigma_obs", "sigma_sq_obs"))
```
...
```{r}
tidyMCMC(m, pars = "beta", conf.int = T, ess = T, rhat = T, estimate.method = "mean") %>% 
    ...
    labs(title = "Daily increase on logit scale",
         subtitle = "Shown as cases per 1000") +
    scale_y_log10() +
    ggsave("hradi.png", width = 8, height = 0.621 * 8, scale = 1.5)
```

## Saturation

```{r}
tidyMCMC(m, pars = c("beta_a", "beta_b","maximum"), conf.int = T, ess = T, rhat = T, estimate.method = "mean")
```
...
tidyMCMC(m, pars = c("alpha", "beta", "maximum"), conf.int = T) %>% 
    ...
    mutate(intercept = log(min_case_rate / (maximum - min_case_rate)),
           ...
           date = date + days) %>% 
    select(-intercept, -linear, -beta, -maximum, -min_case_rate, -alpha) %>% 
...
```{r}
country_id = c(10, 11)
results <- spread_draws(m, beta[country_id], maximum[country_id]) %>% 
    filter(country_id == 1)

min_rate <- d %>% filter(country == "Iceland") %>% .$case_rate %>% min
results %>% 
    sample_n(1000) %>% 
    mutate(min_rate = min_rate,
           intercept = log(min_rate / (maximum - min_rate))) %>% 
    expand_grid(days = seq(0, 40)) %>% 
    mutate(linear = intercept + beta * days,
           rate = maximum * 1 / (1 + exp(-linear))) %>% 
    group_by(days) %>% 
    summarise(estimate = mean(rate),
              upper = quantile(rate, .8)) %>% 
    ggplot(aes(days, estimate)) +
    geom_line() +
    geom_point(data = d %>% filter(country == "Iceland"), aes(days, case_rate), inherit.aes = F) +
...
results %>% 
    ...
    mutate(linear = alpha + beta * days,
           rate = maximum * 1 / (1 + exp(-linear))) %>% 
    ggplot(aes(days, rate, group = iter)) +
    geom_line(alpha = 0.1) +
    scale_y_log10()
```
