...
model {
  ...
  //  Likelihood
  new_deaths ~ neg_binomial_2_log(log_dfdt + log_pop, phi[country]);
}
...
generated quantities {
  ...
  for (i in 1:N_obs) log_lik[i] = neg_binomial_2_log_lpmf(new_deaths[i] | log_dfdt[i] + log_pop[i], phi[country[i]]);
}
