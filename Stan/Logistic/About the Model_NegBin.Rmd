...
* Reparametrized the prior for S in terms of $\mu$ the mean and $\kappa$ the sample size.
...
where we parametrize in terms of the mean, $\mu_S$, and prior sample size, $\kappa_S$
...
* Only use data for countries with a population above 100.000.
...
* Only keep data for countries with more than 11 days of follow-up after the above steps.
...
* Four chains were run for 2000 iterations after a warm-up period of 1000 iterations. The slowest chain converged in 430 seconds. 
...
We have examined the model carefully to find what causes these divergent transitions, and our current belief is country-specific correlations between $\beta_i$ and $S_i$ when the model predicts a high probability of the country being close to its maximum.
