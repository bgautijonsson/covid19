...
```{r, context = "data"}
d <- read_csv("stan_data_GenLogistic.csv")
m <- read_rds("Hierarchical_Model_GenLogistic.rds")
```
...
iceland_d %>% 
    select(date, case_rate) %>% 
    mutate(case_rate = case_rate / 1000) %>% 
    expand_grid(results) %>% 
    mutate(days = as.numeric(date - min(date)),
           z = alpha + days * beta,
           fp = log(case_rate / (S - case_rate))) %>% 
    filter(iter >= max(iter) - 1000) %>% 
    ggplot(aes(z, fp)) +
    geom_line(aes(group = iter), alpha = 0.1) +
    geom_abline(intercept = 0, slope = 1, size = 2, lty = 2) +
    coord_cartesian(xlim = 6 * c(-1, 1), ylim = 6 * c(-1, 1)) +
    xlab(latex2exp::TeX("$\\alpha + \\beta \\cdot t$")) +
    ylab(latex2exp::TeX("$\\log\\left( \\frac{P_{t}}{S - P_{t}}\\right)$")) +
    labs(title = "Fisher-Pry transform applied to observed Icelandic Data",
         subtitle = "Comparing predictions to observations on the logit scale")
```
...
pred_plot <-  eventReactive(input$gobutton, {
  ...
  p1 <- results %>% 
    ...
    filter(date <= max(plot_dat$date) + 28,
           days > 0) %>% 
    group_by(date) %>% 
  ...
  p4 <- results %>% 
    ...
    geom_line(aes(y = lower), lty = 2) +
    geom_line(aes(y = upper), lty = 2) +
    ggtitle(latex2exp::TeX("$R_t$ based on predicted growth and a Gamma(30, 2.5) serial interval"),
