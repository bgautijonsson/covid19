...
```{r, context = "data"}
d <- read_csv("stan_data_GenLogistic_Deaths.csv")
...
fluidRow(
  ...
  column(2,
         numericInput(inputId = "days",
                      label = "Prediction length", min = 1, max = 100, value = 28, step = 1)
  )
)
...
pred_plot <-  eventReactive(input$gobutton, {
  ...
  alph <- 1 - input$conf
  start_cases <- min(plot_dat$total_deaths)
  ...
  results <-  res %>% 
    ...
    expand_grid(days = seq(-20, days_in_data + input$days)) %>% 
    mutate(z = nu * (alpha + beta * days),
           f = S / (1 + exp(-z))^(1/nu),
  ...
  p1 <- results %>% 
    ...
    geom_point(data = plot_dat,
               aes(date, new_deaths), inherit.aes = F) +
  ...
  plot_lab2 <- str_c("Cumulative deaths in  ", 
                     input$country, 
                     ": Prediction and observations")
  ...
  p2 <- results %>% 
    ...
    geom_line() +
    geom_point(data = plot_dat,
               aes(date, total_deaths), inherit.aes = F) +
    scale_y_continuous(breaks = pretty_breaks(8)) +
    labs(y = "Count",
         title = plot_lab2,
         subtitle = "Predicted median and PI") +
