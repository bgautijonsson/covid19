...
fluidRow(
  ...
  column(3,
         numericInput(inputId = "recovered_days",
                      label = "Length of sickness", min = 5, max = 31, value = 21, step = 1)
  ),
  column(3,
         selectInput(inputId = "scale",
                     label = "Scale", choices = c("Original", "Log"), selected = "Original")
  )
)
div(actionButton(inputId = "gobutton", label = "Predict"),
...
pred_plot <-  eventReactive(input$gobutton, {
  ...
  results <- spread_draws(m, alpha[country], beta[country], maximum[country]) %>% 
    ...
    expand_grid(days = seq(-30, days_in_data + 40)) %>% 
