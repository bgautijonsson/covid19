...
```{r}
make_pred <- function(cases, n = 1000, q = 0.95) {
  cases <- rmultinom(n = n, size = cases, prob = aldur$p_tilfelli)
  
  hospital <- matrix(0, nrow = nrow(cases), ncol = ncol(cases))
  serious <- matrix(0, nrow = nrow(cases), ncol = ncol(cases))
  
  for (i in seq_len(ncol(cases))) {
...
if (!file.exists("simulations.csv")) {
  simulations <- preds %>% 
    rowwise %>% 
    mutate(simulation = make_pred(upper, n = 4000)) %>% 
    unnest(simulation) %>% 
